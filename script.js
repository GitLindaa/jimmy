(() => {
  ...
  function openModal(type, id) {
    ...
    const hasPersonal = type === "job" && item.personal;
    if (extraSchoolBlock) extraSchoolBlock.hidden = type !== "school" && !hasPersonal;

    const label4 = $("[data-modal-label-4]");
    if (label4) label4.textContent = hasPersonal ? "Waarom past dit misschien bij jou?" : "Sfeer";

    const body4 = $("[data-modal-body-4]");
    if (body4) {
      if (hasPersonal) {
        body4.textContent = item.personal;
      } else {
